...
**Known work item carried in from Task 2's review — the countable-lap predicate needs a leaf module.** Task 2 extracted `isCountableLap` / `isCountableLapMs` into `track-days.ts` and collapsed `dayBestLapMs` and `SessionProgressionStrip.tsx:175` onto it. `analytics-v2.ts:163` could NOT follow: `track-days.ts` already imports `sessionConsistencySeconds` from `analytics-v2`, so importing back closes a module cycle. Sweep item 1 below ("`isCountableLap` is the only lap predicate") therefore cannot pass until the value-level predicate moves to a leaf. `analytics-constants.ts` imports nothing and is already imported by `analytics-v2.ts:1` — it is the obvious home. Remaining copies to collapse once it lands: `analytics-v2.ts:163`, `data/coachDashboard.ts:102` and `:117`, `components/import/CsvImport.tsx:461`. (`api/coaching/generate/route.ts:163` is Task 5's.) Each is the identical `!== null && > 0` test; if collapsing one changes an existing test's expectations, stop and report rather than editing the expectation.

**Two work items carried in from Task 5's review — `AICoachingCard` still describes the legacy contract.** Task 5 replaced the session prompt's sections, and the card was deliberately left alone (it is not in Task 5's file list, and legacy `ai_coaching_summary` rows still carry the old headings, which the design doc says to leave untouched). Both fixes are small and must actually land here:
- `AICoachingCard.tsx:50-58` — `parseCoaching`'s colour map keys on "Strength" / "Improvement" / "Goal". None of the three new observation headings match, so they fall through to `text-muted` (#9CA3AF) while their own body copy is `text-primary/90` (#E5E7EB): **the heading renders dimmer than the text under it.** Add the new headings without dropping the legacy keys — old rows must keep rendering.
- `AICoachingCard.tsx:107` — `data.error?.includes('not found')` maps to "Session or laps not found". Task 5's corruption 500 originally read `'Track day not found for this session — data integrity issue'`, which contains "not found", so corruption surfaced to the coach as a benign missing-data message. Task 5's review fixes renamed the route's string to `'Track day **missing** for this session — data integrity issue'` (matching its own log line, and pinned by a test that asserts the message does NOT contain "not found") — so the collision is gone from the route side. The defect itself is not: substring-matching another route's prose still stands, and the next 500 whose wording happens to contain "not found" reintroduces it. Match on something the route owns.

**A third work item carried in from Task 5's review — `assessedItemIds` is assembled three times.** The same six lines ("which items were assessed AT this session", the input `focusItemsForSession` takes) appear at `api/coaching/generate/route.ts:197-203`, `components/sessions/EvidenceBanner.tsx:48-52` and `components/days/DebriefSheet.tsx:405-409`. Not a two-views violation in the sense sweep item 1 means — the eligibility RULE lives in the lib and all three call it — but it is the input the rule is only correct for, and a fourth caller will copy whichever of the three it finds. Deliberately NOT collapsed inside Task 5 (a shared helper crossing the route/component boundary is a design call, not a review fix). Decide here: either extract it beside `focusItemsForSession` in `track-days.ts` and collapse all three, or record why three copies is the right answer and stop rediscovering it.
