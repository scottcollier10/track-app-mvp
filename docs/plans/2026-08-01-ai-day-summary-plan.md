...
- `AICoachingCard.tsx:107` — `data.error?.includes('not found')` maps to "Session or laps not found". Task 5's new `'Track day not found for this session — data integrity issue'` contains "not found", so a corruption 500 surfaces to the coach as a benign missing-data message. Substring-matching another route's prose is the actual defect; match on something the route owns.
