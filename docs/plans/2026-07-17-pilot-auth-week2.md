...
- ~~Regenerating `src/lib/types/database.ts` (remove `as any` casts)~~ **RESOLVED 2026-07-19**: PR #16 — `database.ts` regenerated from live schema (`supabase gen types`, generated-only now, header documents the regen command), stale unused `types/supabase.ts` twin deleted, ~60 Supabase `as any` casts stripped across 20 files with zero new casts (design: `docs/plans/2026-07-19-regen-database-types-design.md`). ~~Follow-up candidates from review: residual `: any` parameter annotations downstream of queries (page.tsx, coachDashboard.ts, driverProgress.ts, comparison route) discard the new types; one `as unknown as` shaping cast in `api/drivers/[id]/progress/route.ts:137`~~ **RESOLVED 2026-07-19**: PR #22 — all 38 residual `: any` removed across 15 files (web/src greps clean), dead `DriverComparisonTable-DEBUG.tsx` deleted, progress/route.ts hand-rolled `SessionWithTrack` + all three casts deleted (supabase-js 2.81 infers the tracks embed non-null). New follow-ups from that review: `@types/jest` missing from package.json (241 pre-existing tsc error lines in 4 test files); `(block as any).text` in `api/coaching/generate/route.ts:212` → narrow with `block.type === 'text'`. `driver_profiles.experience_level`/`total_sessions` nullable at DB level: DEFERRED (Scott, 2026-07-19) — batch NOT NULL + defaults with the next real schema migration.
- ~~Fixing the 17 stale insights tests~~ **RESOLVED 2026-07-19**: PR #19 — 14 tests were stale (asserting pre-Dec-2024 capitalized labels; implementation intentionally moved to lowercase enums in 8edfb2a), but 3 caught a REAL regression: `.includes('Improving')`-style checks against now-lowercase labels never matched in `lib/insights.ts` (pace-trend detail never rendered), `sessions/[id]/page.tsx` (trend color always gray), and `lib/queries/driver-progress.ts` (trend always "stable"). All three fixed; **full suite now green 78/78, no exclusions**. ~~Follow-up candidate: <6-lap sessions show label "Stable" (the `|| 'Stable'` fallback), which reads as a trend claim rather than "not enough data".~~ **RESOLVED 2026-07-19**: PR #21 — fallback label is now 'Not enough data' with its own detail string ("needs at least 6 laps"); all four label consumers verified (strict-equality matching falls through to gray badge; LLM prompt wording improved as a side effect).
- ~~`users` table cleanup (likely dead)~~ **RESOLVED 2026-07-19**: PR #20 — confirmed dead (zero code refs, no FKs in/out, 0 rows), dropped from prod via SQL editor + `20260719_drop_users_table.sql`; RLS migration users references removed; `database.ts` regenerated (10 tables).
...
- ~~Regenerate `web/src/reference/track-app-supabase-schema_v2.4.txt` — stale vs prod (claims global UNIQUE on drivers.email; prod has UNIQUE(coach_id, email))~~ **RESOLVED 2026-07-19**: PR #17 — regenerated v2.4 → v3.0 from live catalog via Management API (information_schema + pg_constraint); shows UNIQUE(coach_id, email), llm_logs, auth_user_id, ON DELETE clauses; refreshed post-users-drop (10 tables).
- ~~Custom SMTP for auth emails — Supabase built-in email service rate-limits to a few emails/hour, which blocks multi-coach pilot logins (hit 2026-07-18)~~ **RESOLVED 2026-07-19**: Gmail SMTP (smtp.gmail.com:465, Workspace app password, sender noreply@ alias) configured in Supabase; magic link verified end to end. Post-SMTP redirect scare resolved same day: templates were correct all along; one link was generated with `RedirectTo` falling back to Site URL root (likely stale cached page or non-prod origin at request time — failed allowlist match). Fresh link from the prod login page → `/auth/confirm` → `/coach` verified. Diagnostic that settled it: inspect the actual href in the received email — root+token_hash = allowlist fallback.
- ~~Show logged-in coach identity in the header~~ **RESOLVED 2026-07-19**: PR #12 — root layout + `CoachIdentityProvider` context, verified in prod. ~~Follow-up candidate: no `global-error.tsx` exists (root-layout throw would render Next's raw 500 page).~~ **RESOLVED 2026-07-19**: PR #18 — `global-error.tsx` added (inline styles, logs caught error, contact email + reset).
