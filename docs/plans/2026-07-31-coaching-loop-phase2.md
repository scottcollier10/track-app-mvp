...
2. **PATCH /api/focus-items/[id]** `{ status? , text? }` — at least one. Validate status against the four values; reject empty text. **NEVER writes `updated_at`** (amended 2026-07-31: the Task 2 migration added a `set_updated_at` BEFORE UPDATE trigger — the DB owns that column; a route writing it too is two writers of one field, and a skewed client clock could make `updated_at < created_at`). Status transitions carry NO session anchor and create NO assessment row.
3. **PUT /api/focus-items/[id]/assessments** `{ sessionId, judgment, note? }` — upsert with `{ onConflict: 'focus_item_id,session_id' }`. This is the correction path AND the creation path: one write, the cell. Validate judgment enum. Two contract rules:
   ...
   - **Note preservation:** the upsert payload's columns are exactly what `ON CONFLICT DO UPDATE SET` overwrites. Build the payload conditionally: **omit the `note` key entirely when the request body omits it** (preserves the existing note on a judgment-only correction); include `note: null` only when the body explicitly sends null (clears it). Payload always includes `focus_item_id, session_id, judgment` — and **NEVER `updated_at`** (amended 2026-07-31: the trigger owns it; see route 2). The correction marker is DB-authored: the upsert's DO UPDATE fires the trigger, so a corrected cell gets `updated_at > created_at` without the route doing anything.
